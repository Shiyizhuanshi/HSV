...
proof (induction n arbitrary: ds rule: digits10.induct)
  case (1 n)
  then show ?case 
  proof (cases" n < 10")
    case True
  ...
  next
    case False
    then have "ds = (n mod 10) # digits10 (n div 10)" using "1.prems" by simp
    moreover have "n mod 10 < 10" by simp
    moreover have "\<forall>d \<in> set (digits10 (n div 10)). d < 10" using "1.IH" False by blast
...
theorem ababab_37_divisible:
  fixes a b :: nat
  assumes "a < 10" "b < 10"
  defines "n \<equiv> 101010 * a + 10101 * b"
  shows "37 dvd n"
proof -
  have "n = (10 * a + b)* 10101" using n_def by simp
